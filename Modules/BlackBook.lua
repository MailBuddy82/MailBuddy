...
local Postal_BlackBook = MailBuddy:NewModule("BlackBook", "AceEvent-3.0", "AceHook-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("MailBuddy")
MailBuddy_BlackBook.description = L["Adds a contact list next to the To: field."]
MailBuddy_BlackBook.description2 = L[ [[|cFFFFCC00*|r This module will list your contacts, friends, guild mates, alts and track the last 10 people you mailed.
...
local MailBuddy_BlackBookButton
...
local MailBuddy_BlackBook_Autocomplete_Flags = {
	include = AUTOCOMPLETE_FLAG_ALL,
	exclude = AUTOCOMPLETE_FLAG_BNET,
}

function Postal_BlackBook:OnEnable()
	if not MailBuddy_BlackBookButton then
		-- Create the Menu Button
		MailBuddy_BlackBookButton = CreateFrame("Button", "MailBuddyl_BlackBookButton", SendMailFrame)
		MailBuddy_BlackBookButton:SetWidth(25)
		MailBuddy_BlackBookButton:SetHeight(25)
		MailBuddy_BlackBookButton:SetPoint("LEFT", SendMailNameEditBox, "RIGHT", -2, 2)
		MailBuddy_BlackBookButton:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Up")
		MailBuddy_BlackBookButton:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Round")
		MailBuddy_BlackBookButton:SetDisabledTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Disabled")
		MailBuddy_BlackBookButton:SetPushedTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Down")
		MailBuddy_BlackBookButton:SetScript("OnClick", function(self, button, down)
			if Postal_DropDownMenu.initialize ~= MailBuddy_BlackBook.BlackBookMenu then
				CloseDropDownMenus()
				MailBuddy_DropDownMenu.initialize = MailBuddy_BlackBook.BlackBookMenu
			end
			ToggleDropDownMenu(1, nil, MailBuddy_DropDownMenu, self:GetName(), 0, 0)
		end)
		MailBuddy_BlackBookButton:SetScript("OnHide", MailBuddy_DropDownMenu.HideMenu)
	end
