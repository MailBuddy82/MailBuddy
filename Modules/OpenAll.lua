...
local MailBuddy_OpenAll = MailBuddy:NewModule("OpenAll", "AceEvent-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("MailBuddy")
MailBuddy_OpenAll.description = L["A button that collects all attachments and coins from mail."]
MailBuddy_OpenAll.description2 = L[ [[|cFFFFCC00*|r Simple filters are available for various mail types.
...
local wait
local button
local MailBuddy_OpenAllMenuButton
local skipFlag
...
updateFrame:SetScript("OnShow", function(self)
	self.time = MailBuddy.db.profile.OpenSpeed
...
refreshFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	if self.time <= 0 then
		if self.mode == nil then
			self.time = 10
			MailBuddy:Print(L["Refreshing mailbox..."])
			self:RegisterEvent("MAIL_INBOX_UPDATE")
			CheckInbox()
			refreshFrame:OnEvent()
		else
			self:Hide()
			MailBuddy_OpenAll:OpenAll(true)
		end
	end
end)
...
function MailBuddy_OpenAll:OnEnable()
	if not button then
		...
		button:SetScript("OnClick", function() MailBuddy_OpenAll:OpenAll() end)
		button:SetFrameLevel(button:GetFrameLevel() + 1)
	end
	if not Postal_OpenAllMenuButton then
		-- Create the Menu Button
		MailBuddy_OpenAllMenuButton = CreateFrame("Button", "Postal_OpenAllMenuButton", InboxFrame);
		MailBuddy_OpenAllMenuButton:SetWidth(30);
		MailBuddy_OpenAllMenuButton:SetHeight(30);
		MailBuddy_OpenAllMenuButton:SetPoint("LEFT", button, "RIGHT", -2, 0);
		MailBuddy_OpenAllMenuButton:SetNormalTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Up");
		MailBuddy_OpenAllMenuButton:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Round");
		MailBuddy_OpenAllMenuButton:SetDisabledTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Disabled");
		MailBuddy_OpenAllMenuButton:SetPushedTexture("Interface\\ChatFrame\\UI-ChatIcon-ScrollDown-Down");
		MailBuddy_OpenAllMenuButton:SetScript("OnClick", function(self, button, down)
			if MailBuddy_DropDownMenu.initialize ~= MailBuddy_OpenAll.ModuleMenu then
				CloseDropDownMenus()
				MailBuddy_DropDownMenu.initialize = MailBuddy_OpenAll.ModuleMenu
			end
			ToggleDropDownMenu(1, nil, MailBuddy_DropDownMenu, self:GetName(), 0, 0)
		end)
		MailBuddy_OpenAllMenuButton:SetFrameLevel(MailBuddy_OpenAllMenuButton:GetFrameLevel() + 1)
	end
	...
	button:Show()
	MailBuddy_OpenAllMenuButton:SetScript("OnHide", MailBuddy_DropDownMenu.HideMenu)
	MailBuddy_OpenAllMenuButton:Show()
end

function MailBuddy_OpenAll:OnDisable()
	self:Reset()
	button:Hide()
	OpenAllMail:Show() -- show Blizzard's Open All button
	MailBuddy_OpenAllMenuButton:SetScript("OnHide", nil)
	MailBuddy_OpenAllMenuButton:Hide()
end

function MailBuddy_OpenAll:MAIL_SHOW()
	self:RegisterEvent("MAIL_CLOSED", "Reset")
	self:RegisterEvent("PLAYER_LEAVING_WORLD", "Reset")
end

function MailBuddy_OpenAll:OpenAll(isRecursive)
	refreshFrame:Hide()
...
function MailBuddy_OpenAll:ProcessNext()
	...
	if mailIndex > 0 then
		...
		if wait then
			local attachCount, goldCount = MailBuddy:CountItemsAndMoney()
		...
		-- Filter by mail type
		local mailType = MailBuddy:GetMailType(msgSubject)
		if mailType == "NonAHMail" then
			-- Skip mail with attachments according to user options
			if msgItem and MailBuddy.db.profile.OpenAll.Postmaster and sender
			...
			elseif openAllOverride or MailBuddy.db.profile.OpenAll.Attachments then
			...
			if not (openAllOverride or MailBuddy.db.profile.OpenAll[mailType]) then
				mailIndex = mailIndex - 1
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext() -- tail call
			end
		...
		if MailBuddy.db.profile.OpenAll.SpamChat and attachIndex == ATTACHMENTS_MAX_RECEIVE then
			if not invFull or msgMoney > 0 then
				local moneyString = msgMoney > 0 and " ["..MailBuddy:GetMoneyString(msgMoney).."]" or ""
				local playerName
				...
				MailBuddy:Print(format("%s %d: %s%s%s", L["Processing Message"], mailIndex, msgSubject or "", moneyString, (playerName or "")))
			end
		end
		...
		if attachIndex > 0 and not invFull and MailBuddy.db.profile.OpenAll.KeepFreeSpace>0 then
			local free=0
			...
			if free <= MailBuddy.db.profile.OpenAll.KeepFreeSpace then
				invFull = true
				invAlmostFull = nil
				MailBuddy:Print(format(L["Not taking more items as there are now only %d regular bagslots free."], free))
			elseif free <= MailBuddy.db.profile.OpenAll.KeepFreeSpace + 1 then
				invAlmostFull = true
			end
		end
		...
		if attachIndex > 0 and (lootFlag or not invFull) then
			...
			--MailBuddy:Print("Getting Item from Message "..mailIndex..", "..attachIndex)
			lastNumAttach, lastNumGold = MailBuddy:CountItemsAndMoney()
		...
		elseif msgMoney > 0 then
			...
			--MailBuddy:Print("Getting Gold from Message "..mailIndex)
			lastNumAttach, lastNumGold = MailBuddy:CountItemsAndMoney()
		...
		elseif totalItems > numItems and numItems < MAX_MAIL_SHOWN then
			...
			MailBuddy:Print(L["Not all messages are shown, refreshing mailbox soon to continue Open All..."])
			refreshFrame:Show()
			return
		end

		if skipFlag then MailBuddy:Print(L["Some Messages May Have Been Skipped."]) end
		self:Reset()
	end
end

function MailBuddy_OpenAll:Reset(event)
	refreshFrame:Hide()
	updateFrame:Hide()
	self:UnregisterEvent("UI_ERROR_MESSAGE")
	button:SetText(L["Open All"])
	MailBuddy:DisableInbox()
	InboxFrame_Update()
...
function MailBuddy_OpenAll:UI_ERROR_MESSAGE(event, error_message)
...
function MailBuddy_OpenAll.SetKeepFreeSpace(dropdownbutton, arg1)
	MailBuddy.db.profile.OpenAll.KeepFreeSpace = arg1
end

function MailBuddy_OpenAll.ModuleMenu(self, level)
	if not level then return end
	local info = self.info
	wipe(info)
	info.isNotRadio = 1
	local db = MailBuddy.db.profile.OpenAll
	...
	elseif level == 3 + self.levelAdjust then
		if UIDROPDOWNMENU_MENU_VALUE == "KeepFreeSpace" then
			...
			info.func = MailBuddy_OpenAll.SetKeepFreeSpace
			info.isNotRadio = nil
			for _, v in ipairs(MailBuddy.keepFreeOptions) do
				info.text = v
