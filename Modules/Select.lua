...
local MailBuddy_Select = MailBuddy:NewModule("Select", "AceEvent-3.0", "AceHook-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("MailBuddy")
MailBuddy_Select.description = L["Add check boxes to the inbox for multiple mail operations."]
MailBuddy_Select.description2 = L[ [[|cFFFFCC00*|r Selected mail will be batch opened or returned to sender by clicking Open or Return.
...
local checkboxFunc = function(self) MailBuddy_Select:ToggleMail(self) end
...
local function printTooMuchMail()
	...
	if cur>=100 or -- if inbox is full, no more will arrive
	   timeLeft<0 then	-- if someone waited more than 60 seconds to take a mail out....
		MailBuddy:Print(format(L["There are %i more messages not currently shown."], tot-cur))
	else
		MailBuddy:Print(format(L["There are %i more messages not currently shown. More should become available in %i seconds."], tot-cur, timeLeft))
	end

end

function MailBuddy_Select:OnEnable()
	--create the open button
	if not openButton then
		openButton = CreateFrame("Button", "MailBuddySelectOpenButton", InboxFrame, "UIPanelButtonTemplate")
		openButton:SetWidth(120)
		openButton:SetHeight(25)
		openButton:SetPoint("RIGHT", InboxFrame, "TOP", 0, -42)
		openButton:SetText(L["Open"])
		openButton:SetScript("OnClick", function() MailBuddy_Select:HandleSelect(1) end)
		openButton:SetFrameLevel(openButton:GetFrameLevel() + 1)
	end

	--create the return button
	if not returnButton then
		returnButton = CreateFrame("Button", "MailBuddySelectReturnButton", InboxFrame, "UIPanelButtonTemplate")
		...
		returnButton:SetScript("OnClick", function() MailBuddy_Select:HandleSelect(2) end)
	...
	for i = 1, 7 do
		if not _G["MailBuddyInboxCB"..i] then
			local CB = CreateFrame("CheckButton", "MailBuddyInboxCB"..i, _G["MailItem"..i], "OptionsCheckButtonTemplate")
			CB:SetID(i)
			...
			local text = CB:CreateFontString("MailBuddyInboxCB"..i.."Text", "BACKGROUND", "GameFontHighlightSmall")
	...
	returnButton:Show()
	for i = 1, 7 do
		_G["MailBuddyInboxCB"..i]:Show()
	end
end

function MailBuddy_Select:OnDisable()
	self:Reset()
...
function MailBuddy_Select:MAIL_SHOW()
...
function MailBuddy_Select:ToggleMail(frame)
...
function MailBuddy_Select:GetUniqueID(index)
...
function MailBuddy_Select:BuildUniqueIDs()
...
function MailBuddy_Select:HandleSelect(mode)
	...
	for i = 1, 7 do
		...
		local CB = _G["MailBuddyInboxCB"..i]
		CB:Hide()
	end
...
function MailBuddy_Select:ProcessNext()
	...
	if mailIndex > 0 then
		...
		if currentMode == 1 then
			...
			if MailBuddy.db.profile.Select.SpamChat and attachIndex == ATTACHMENTS_MAX_RECEIVE then
				if not invFull or msgMoney > 0 then
					...
					MailBuddy:Print(format("%s %d: %s%s%s", L["Open"], mailIndex, msgSubject or "", moneyString, (playerName or "")))
				end
			end
			...
			if attachIndex > 0 and not invFull and MailBuddy.db.profile.Select.KeepFreeSpace > 0 then
				local free = 0
				...
				if free <= MailBuddy.db.profile.Select.KeepFreeSpace then
					invFull = true
					invAlmostFull = nil
					MailBuddy:Print(format(L["Not taking more items as there are now only %d regular bagslots free."], free))
				elseif free <= MailBuddy.db.profile.Select.KeepFreeSpace + 1 then
					invAlmostFull = true
				end
			end
			...
			if not InboxItemCanDelete(mailIndex) then
				...
				MailBuddy:Print(L["Skipping"].." "..mailIndex..": "..msgSubject)
				mailIndex = mailIndex - 1
				attachIndex = ATTACHMENTS_MAX_RECEIVE
				return self:ProcessNext() -- tail call
			end
...
function MailBuddy_Select:InboxFrame_Update()
...
function MailBuddy_Select:MAIL_INBOX_UPDATE()
	-- MailBuddy:Print("update")
	...
	if currentFirstMailDaysLeft ~= firstMailDaysLeft then
		...
		--MailBuddy:Print(numNewMailsAtFront.. " new mails detected at front")
...
function MailBuddy_Select:Reset(event)
...
function MailBuddy_Select:UI_ERROR_MESSAGE(event, error_message)
...
function MailBuddy_Select.SetKeepFreeSpace(dropdownbutton, arg1, arg2, checked)
	MailBuddy.db.profile.Select.KeepFreeSpace = arg1
end

function MailBuddy_Select.ModuleMenu(self, level)
	...
	if level == 1 + self.levelAdjust then
		...
		info.arg1 = "Select"
		info.arg2 = "SpamChat"
		info.checked = MailBuddy.db.profile.Select.SpamChat
		info.isNotRadio = 1
		UIDropDownMenu_AddButton(info, level)
	elseif level == 2 + self.levelAdjust then
		if UIDROPDOWNMENU_MENU_VALUE == "KeepFreeSpace" then
			local keepFree = MailBuddy.db.profile.Select.KeepFreeSpace
			info.func = MailBuddy_Select.SetKeepFreeSpace
